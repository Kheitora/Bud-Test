...
function showSensorData(data){
    ...
    raiseTemp = minimaletemperatuur - temperatuurSensor;
    console.log(raiseTemp);
...
function meldingen() {
    ...
    let meldingTemperatuur = () => {
        if (temperatuurSensor >= minimaletemperatuur && temperatuurSensor <= maximaletemperatuur) {
            ...
            message = true
            console.log('alles gaat goed')
        } else if (temperatuurSensor <= minimaletemperatuur) {
            message = true
            document.querySelector("#sensor-bud").innerHTML = "Bud 1: "
            document.getElementById("temp").innerHTML = "Je plant bevriest zo wat!";
            console.log('Je plant bevriest zowat!')
            raiseTemp = minimaletemperatuur - temperatuurSensor;
            document.getElementById("changeTemp").innerHTML = "voor het beste resultaat moet je je thermostaat met minstens " + raiseTemp + " graden celsius verhogen";
        } else if (temperatuurSensor >= maximaletemperatuur) {
            document.querySelector("#sensor-bud").innerHTML = "Bud 1: "
            message = true
            document.getElementById("temp").innerHTML = "Het is veeel te warm voor je plant!";
            console.log("Het is veeel te warm voor je plant!")
            lowTemp = temperatuurSensor - maximaletemperatuur;
            document.getElementById("changeTemp").innerHTML = "Voor het beste resultaat moet je je thermostaat met minstens " + lowTemp + " graden celsius verlagen";
        } else {
            console.log('geen plant');           
        }
    }
